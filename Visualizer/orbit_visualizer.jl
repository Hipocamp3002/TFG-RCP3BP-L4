...
dir_toggle = Toggle(fig, active = false)
...
input_orbit = inputGrid_orbit[1,1:5] = [angle_input,mu_input,calc_button,time_input,dir_toggle]
...
bodies = Observable([[-mu,0,0,-mu],[1-mu,0,0,1-mu]])
...
on(angle_input.stored_string) do s
    global angle = deg2rad(parse(Float64,s))
    empty!(path[])
    calc_orbit()
    notify(path)
end
